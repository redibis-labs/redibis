...
def default_checkout_root() -> Path | None:
    """Directory that contains ``tests/data`` in a source checkout, if any."""
    tests = default_tests_data_path()
    if tests is None:
        return None
    return tests.parent.parent


def _host_relative_files(raw: str):
    """Yield cwd- then checkout-relative paths that already exist as files."""
    candidates: list[Path] = []
    try:
        candidates.append(Path.cwd())
    except (OSError, RuntimeError):
        pass
    checkout = default_checkout_root()
    ...
    for base in candidates:
        ...
        seen.add(key)
        try:
            hit = (base / raw).expanduser().resolve()
        except (OSError, RuntimeError):
            continue
        if hit.is_file():
            yield hit
...
def _resolve_absolute(raw: str, root_name: str, cfg: Any) -> Path:
    try:
...
def resolve(root_name: str, rel_path: str, cfg: Any = None) -> Path:
    ...
    Accepts a name relative to the root (``customers.csv``), a checkout path
    such as ``tests/data/customers.csv``, or an absolute path that still
    resolves inside a configured root. This is the security gate. Callers
    must never build a path themselves.
    """
    cfg = cfg if cfg is not None else _config()
    ...
    if _is_absolute_user_path(raw):
        return _resolve_absolute(raw, root_name, cfg)
    ...
    if root_name:
        try:
            ...
            if not str(exc).startswith("not a file:"):
                raise
            last_missing = exc
    else:
    ...
    for host in _host_relative_files(raw):
        try:
            return _resolve_absolute(str(host), "", cfg)
        except SampleDataError as exc:
            if "outside the sample data root" not in str(exc):
                raise
    if root_name:
        return _resolve_inside_root(get_root(root_name, cfg), parts, cfg)
